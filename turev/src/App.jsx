...
// İzole math.js instance (bundler ile uyumlu)
...
function normalizeExpr(raw) {
  ...
  let s = raw.trim();
  // |x| -> abs(x) (sadece dıştan tek çift dikey çizgiyi çeviriyoruz)
...
// f'(x) derleyici: önce sembolik, olmazsa sayısal fark
...
// ---------- Self Tests ----------
// (Mevcut testleri değiştirmiyoruz; ek testler dahil)
...
export default function App() {
  ...
  useEffect(() => {
    if (!f) {
      setError(
        "İfade yorumlanamadı. math.js sözdizimi kullan: sin(x), cos(x), tan(x), exp(x), log(x), abs(x), ^ (üs). Parça tanım ör.: x<0?-x:x^2"
      );
      return;
    }
    setError("");
  ...
  // Basit stiller (Tailwind yoksa da şık görünsün)
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", display: "grid", gap: 16 }}>
        <header style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "flex-end", justifyContent: "space-between" }}>
          <div>
            ...
            <p style={{ margin: "6px 0", opacity: 0.8 }}>Üst panel: f(x), alt panel: f'(x). İmleçte teğet çizgisi.</p>
          </div>
        ...
        <section style={box}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 8, marginTop: 10 }}>
            ...
            <div style={{ ...box, padding: 8 }}>
              ...
              <input
                type="number"
                value={samples}
                onChange={(e) => setSamples(clamp(parseInt(e.target.value || "0"), 100, 4000))}
                style={{ width: "100%", padding: 6, borderRadius: 8, border: "1px solid #cbd5e1" }}
              />
            </div>
          </div>
          ...
          {error && (
            <div style={{ marginTop: 10, padding: 10, borderRadius: 10, background: "#fff1f2", color: "#9f1239", border: "1px solid #fecdd3" }}>
              {error}
            </div>
          )}
          ...
          <div style={{ fontSize: 12, opacity: 0.8, marginTop: 8 }}>
            ...
            <br />
            Not: Türev mümkünse sembolik, değilse sayısal merkez farkı ile. Köşe/kesiklik noktalarında teğet gizlenir.
          </div>
        </section>

        <section style={box}>
          <h2 style={{ fontSize: 14, fontWeight: 700, margin: 0, marginBottom: 8 }}>Kendini test (otomatik)</h2>
          <ul style={{ fontSize: 12, margin: 0, paddingLeft: 18 }}>
            {tests.map((t) => (
              <li key={t.name} style={{ color: t.pass ? "#047857" : "#be123c", marginBottom: 4 }}>
                {t.pass ? "PASS" : "FAIL"} — {t.name} <span style={{ color: "#94a3b8" }}>({t.note})</span>
              </li>
            ))}
          </ul>
        </section>
